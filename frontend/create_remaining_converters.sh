...
# Create WEBP to JPG
cat > webp-to-jpg.html << 'HTML'
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>WEBP to JPG Converter - SIGIRI</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-XXXXXXXXXXXXXXXX" crossorigin="anonymous"></script>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, sans-serif; background: linear-gradient(135deg, rgba(79, 172, 254, 0.1) 0%, rgba(0, 242, 254, 0.1) 100%); backdrop-filter: blur(100px); min-height: 100vh; display: flex; position: relative; overflow-x: hidden; }
        body::before { content: ''; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: radial-gradient(circle at 20% 30%, rgba(79, 172, 254, 0.15) 0%, transparent 50%); z-index: 0; pointer-events: none; }
        ...
        .shape1 { width: 500px; height: 500px; background: linear-gradient(135deg, rgba(79, 172, 254, 0.25) 0%, rgba(0, 242, 254, 0.25) 100%); top: 50%; right: 10%; animation-delay: 3s; }
        .shape2 { width: 450px; height: 450px; background: linear-gradient(135deg, rgba(67, 233, 123, 0.25) 0%, rgba(56, 249, 215, 0.25) 100%); bottom: 20%; left: 15%; animation-delay: 10s; }
        ...
        .converter-header h1 { font-size: 2.5rem; margin-bottom: 12px; background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
        .converter-header p { color: #666; font-size: 1.1rem; }
        .upload-area { background: rgba(255, 255, 255, 0.75); backdrop-filter: blur(25px); border: 3px dashed rgba(79, 172, 254, 0.3); border-radius: 16px; padding: 60px 40px; text-align: center; cursor: pointer; transition: all 0.3s; margin-bottom: 30px; }
        .upload-area:hover { border-color: #4facfe; transform: translateY(-5px); box-shadow: 0 15px 35px rgba(0, 0, 0, 0.1); }
        .upload-area i { font-size: 4rem; color: #4facfe; margin-bottom: 20px; }
        .upload-area h3 { font-size: 1.5rem; color: #333; margin-bottom: 10px; }
        .upload-area p { color: #666; font-size: 1rem; }
        .file-input { display: none; }
        .convert-btn { background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); color: white; border: none; padding: 16px 48px; border-radius: 12px; font-size: 1.1rem; font-weight: 600; cursor: pointer; width: 100%; transition: all 0.3s; box-shadow: 0 8px 25px rgba(79, 172, 254, 0.3); }
        .convert-btn:hover { transform: translateY(-2px); box-shadow: 0 12px 35px rgba(79, 172, 254, 0.3); }
        ...
        .feature-card i { font-size: 2.5rem; color: #4facfe; margin-bottom: 16px; }
...
<body>
    ...
    <div class="main-container">
        <div class="content">
            ...
            <div class="converter-header"><h1>WEBP to JPG Converter</h1><p>Convert modern WEBP format to standard JPG images</p></div>
            <div class="upload-area" onclick="document.getElementById('fileInput').click()"><i class="fas fa-images"></i><h3>Drop your WEBP files here</h3><p>or click to browse</p><input type="file" id="fileInput" class="file-input" accept=".webp" multiple></div>
            <button class="convert-btn" disabled><i class="fas fa-sync-alt"></i> Convert to JPG</button>
            ...
            <div class="features">
                <div class="feature-card"><i class="fas fa-bolt"></i><h4>Fast Conversion</h4><p>Lightning-fast WEBP to JPG conversion</p></div>
